...
(starting at v15.1.0) adds two gates beyond the four-gate semantic
stack of \S\ref{sec:fourgate}:

\begin{itemize}[noitemsep, topsep=2pt]
\item \textbf{No-meaningful-relation gate.} If the chain's first
      hop is not a ``lifting'' relation and the chain is not
      same-relation transitive, no specific relation can be
      ascribed and the induction is rejected. Storing the chain
      under a generic \texttt{implies} fallback is opt-in
      (\texttt{InductionPolicy(allow\_generic\_implies=True)}) and
      off by default.
\item \textbf{Type-signature gate.} Each lifting-relation family
      permits only a specific set of last-hop relations. \texttt{partof}
      transfers $\{$\texttt{locatedin, madeof, usedfor}$\}$ (the
      part inherits the whole's location, material, and use), but
      not the whole's \texttt{has} relation
      (a wing doesn't have what the bird has).
      \texttt{locatedin} transfers only aggregate-location relations
      $\{$\texttt{continent, country, region, city}$\}$, not the
      location's \texttt{size}, \texttt{color}, or \texttt{isa}
      attributes. \texttt{isa}/\texttt{class}/\texttt{category} and
      siblings transfer $\{$\texttt{isa, has, kind, class,
      category}$\}$.
\end{itemize}
...
\texttt{(greatexpectations, author, olivertwist)}, etc). The
four-gate semantic stack eliminated inverse-pair and cycle errors;
the two additional gates introduced here eliminate the
type-confusion errors that the four-gate stack let through. We
report ``31/31 manually-validated under the v15.1.0 pipeline''
rather than ``100\% precision''---the latter is what an empirical
claim looks like when the test corpus is small and adversarial
probes haven't been built. A larger external benchmark (e.g.\
applying the pipeline to a 10{,}000-probe sample of Wikidata) is
future work.
